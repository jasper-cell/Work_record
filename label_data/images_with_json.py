...
imagepath_list = glob.glob("./DaTi_mask1/*/img.png", recursive=True)
print(imagepath_list)
maskpath_list = glob.glob("./DaTi_mask1/*/label.png", recursive=True)
print(maskpath_list)
...
for (imagePath, maskPath) in zip(imagepath_list, maskpath_list):
    ...
    # cv2.waitKey(0)
    cv2.imwrite("./imgs_with_json/{}.png".format(image_name), image)
    cv2.imwrite("./imgs_with_json/{}.png".format(mask_name), mask)
