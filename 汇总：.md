...
5. **量化和剪枝**
   ...
   2. 量化
      ...
      1. 模型量化是指将模型**权重参数使用更少的比特数存储**，以此来**减少模型的存储空间和算力消耗**
         1. BERT原本是以fp32精度运行的，若换成fp16精度，则存储空间立刻减少一半，在针对fp16精度优化的运算设备上(英伟达T4\V100等)，运行速度可以得到明显的提升。
         2. 量化是一种通用的压缩方法，适用于几乎所有的深度学习模型
         3. 数据精度降低后，模型的质量肯定也会有所降低，可以通过**量化后训练(QAT)来进行缓解。**

   3. 剪枝

      1. 最简单的一种想法：直接删掉参数矩阵的某一列，对应的就是删掉一个filter，相应的输出特征图将少一个通道。而在下一个卷积操作时，这个输出特征图将变成输入特征图，带入到图1右侧的输入矩阵，少一个通道即少一个列，那么为了保证矩阵乘法正确，参数矩阵必须要对应删去一行，即所有filter都要删去一个通道，整体变化如图2所示。从上述介绍我们发现：若对某层卷积以filter单位裁枝，那么到下一层一定会有以channel单位的裁枝，这就是filter-level裁枝(图4中所示)

         ![image-20210804092550551](C:\Users\ict\AppData\Roaming\Typora\typora-user-images\image-20210804092550551.png)
